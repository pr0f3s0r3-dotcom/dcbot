...
const fs = require("fs");
require('dotenv').config(); // .env'i okur
const express = require("express"); // HTTP server için
...
// ----------------- DOSYALARI YÜKLE -----------------
let profiles = fs.existsSync(PROFILE_FILE) ? JSON.parse(fs.readFileSync(PROFILE_FILE)) : {};
let deletedMessages = fs.existsSync(DELETED_FILE) ? JSON.parse(fs.readFileSync(DELETED_FILE)) : {};
...
client.on("messageCreate", async message => {
  ...
  if (content === "profiles") {
    ...
    if (!userProfiles || userProfiles.length === 0) return message.channel.send("Kayıtlı profil yok.");

    const menu = new StringSelectMenuBuilder()
      .setCustomId("profile_select")
      .setPlaceholder("Profil seç")
      .addOptions(userProfiles.map(p => ({
        label: p.name.length > 100 ? p.name.slice(0, 97) + "..." : p.name,
        value: p.name
      })));

    return message.channel.send({ content: "Profil seç:", components: [new ActionRowBuilder().addComponents(menu)] });
  }

  // Delete profile
  if (content === "delete profile") {
    const userProfiles = profiles[message.author.id];
    if (!userProfiles || userProfiles.length === 0) return message.channel.send("Silinecek profil yok.");
    ...
    return message.channel.send({ content: "Silmek istediğiniz profili seçin:", components: [new ActionRowBuilder().addComponents(menu)] });
  }
  ...
  if (content.startsWith("delete ")) {
    ...
    if (!message.guild) return; // DM'de çalışmaz
    const now = Date.now();
  ...
  if (/^[A-Za-z0-9_-]{78}$/.test(message.content.trim())) {
    ...
    return message.channel.send({ content: "Region seç:", components: [new ActionRowBuilder().addComponents(menu)] });
  }
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.customId === "profile_select") {
    const profile = profiles[interaction.user.id]?.find(p => p.name === interaction.values[0]);
    if (!profile) return interaction.update({ content: "Profil bulunamadı.", components: [] });
  ...
  if (interaction.customId === "delete_profile_select") {
    profiles[interaction.user.id] = profiles[interaction.user.id].filter(p => p.name !== interaction.values[0]);
    fs.writeFileSync(PROFILE_FILE, JSON.stringify(profiles, null, 2));
  ...
  if (interaction.customId === "region_select") {
    ...
    pendingNames.set(interaction.user.id, { puuid, region: interaction.values[0] });
...
client.on("messageDelete", async message => {
  ...
  if (message.guild.id === MAIN_GUILD_ID) {
    targetChannel = message.guild.channels.cache.find(c => c.name === TRASH_CHANNEL_NAME);
  } else {
